...
{
    [TestClass]
    public class TEST_AustenizationTemp
    {
        [TestMethod]
        public void TestCalcAustenizationTempreture()
        {
            float q = -1;
            int objNumber = 1;
            ...
            var results = result.CalcAustenizationTempreture(chemDic);

            Assert.AreEqual(Math.Round(results, 3), 778.054);
        }
    }
}
